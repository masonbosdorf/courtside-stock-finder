...
| `shopify.js` / `shopify-prices.js` | Shopify Admin API (client credentials, shared with the siblings) and the variant pull — price, productType and tags for the kind filters, and the **online light** (green pulse = ACTIVE and published to the Online Store via `onlineStoreUrl`; red = POS-only or unpublished): price = what POS rings up, compareAt > price = on sale. Only ACTIVE products; anything else shows "not on POS". Falls back to the previous seed's prices if Shopify is down |
